...
async function perguntarIA(texto){
  ...
  try {
    ...
    if (d.ativo === false) return semIA(texto);
    if (!d.achou) return naoAchou(texto);
...
/* Sem chave configurada ou sem rede: cai no resolvedor local. */
function semIA(texto){
  const r = resolver(texto, TUDO);
  if (r){
    const b = desenhaBalao("ia", `Não consigo responder em texto agora — a camada de IA não está ligada ou está sem rede. Mas isso está no conteúdo:`);
    const ir = document.createElement("button");
    ir.className = "ir-chat";
    ir.textContent = "Abrir " + r.item.nome;
    ir.onclick = () => abrir(r.item, r);
    b.append(ir);
  } else naoAchou(texto);
}

function naoAchou(texto){
