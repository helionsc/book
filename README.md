...
`testes/interface.test.js` abre o app num navegador simulado e percorre todos
os itens. Precisa de um servidor rodando e do jsdom:
...
Não está no `npm test` porque depende de servidor de pé. Rode antes de
publicar mudanças de layout.

Existe por causa de um bug real: o placar do resultado e a barra de abas são
ambos fixos no rodapé, e a barra de abas ficava por cima. O escore era
calculado corretamente e simplesmente não aparecia — nenhuma suíte de dados
pegaria isso.
...
Segue a preferência do sistema automaticamente. Não há botão: uma decisão a
menos às 3h é melhor que uma opção a mais.

A razão não é estética. Tela branca no escuro cansa a vista e destrói a
adaptação ao escuro — relevante para quem precisa olhar o paciente logo
depois de consultar o app.

O modo ambulância (`A+`) funciona junto com os dois esquemas.

## Tema

Botão de sol no topo, alterna entre **Automático → Claro → Escuro**.

O automático é o padrão, mas a escolha manual existe por um motivo concreto:
plantão tem horário invertido, e o sistema do celular segue o relógio, não o
turno de quem trabalha à noite.

## Layout para uma mão

Ajustes abaixo de 520px de largura:

- Nenhum controle abaixo de 44px de altura — é o mínimo recomendado, e luva
  piora a precisão do toque
- Cabeçalho mais enxuto: 120px fixos de topo comiam metade da primeira tela
- Descrição do cartão limitada a duas linhas, para caber mais itens sem rolar
- Campos numéricos do registro em três colunas em vez de empilhados
- Fonte de campo em 16px, que impede o zoom automático do iOS ao focar
- Breakpoint extra abaixo de 360px (iPhone SE)

Ações frequentes ficam no terço inferior, onde o polegar alcança.

---

## Ficha de parada pediátrica

Integrada de um app avulso. Peso e idade geram: faixa de peso (padrão
Broselow), volumes das 26 drogas, energia de cardioversão e desfibrilação,
sinais vitais esperados, medidas de equipamento e estado nutricional pela
curva de IMC-para-idade da OMS.

Decisões da integração:

- **A tabela da OMS vive em `public/oms-imc.json`** (13 KB) e só é carregada
  quando a ficha abre. Não faz sentido baixar curvas de crescimento para quem
  vai calcular autonomia de cilindro.
- **As fórmulas de dose são texto no `conteudo.json`**, como no resto do app,
  para a revisora poder ajustá-las sem tocar em código.
- **Contexto pediátrico decide a ferramenta.** "Dose de adrenalina pediátrica"
  caía na bomba de infusão de adulto, porque o nome da droga pesava mais que
  a idade. Palavras como criança, lactente e neonato agora têm prioridade —
  é a troca exata que causa dose errada em criança.

### Teste de paridade

`testes/paridade-pediatrica.js` compara cada droga com o HTML original em
seis pesos diferentes, conferindo apresentação, dose, diluente, diluição,
teto e volume calculado:

```bash
node testes/paridade-pediatrica.js caminho/para/ficha-parada-pediatrica.html
```

Existe porque erro de migração em dose pediátrica é silencioso: um fator de
10 trocado parece um número plausível na tela.

---

## Chat — como o contexto é montado

**Busca de contexto.** Antes eu mandava ao modelo só os itens que o resolvedor
de sinônimos acertava; quando ele errava, mandava os de Transporte por padrão,
e o modelo respondia "não sei" sobre coisa que estava no conteúdo. Agora todos
os itens são pontuados pelo texto completo — incluindo nomes de drogas e
critérios internos — e os cinco melhores vão junto com a pergunta.

**Memória curta.** As últimas seis mensagens acompanham cada pergunta, então
"e em 60 kg?" continua o assunto anterior. Dois detalhes que custaram bug:

- O contexto é lido **antes** de empilhar a pergunta. Empilhando primeiro, a
  função encontrava a própria pergunta como assunto anterior.
- Na junção, a pergunta nova vem **primeiro**. A extração de números pega a
  primeira ocorrência, e em "e em 60 kg?" o peso que vale é o novo.

**Perguntas sobre o app** ("o que você sabe?") são respondidas no aparelho,
listando as ferramentas por área. Não gastam chamada de API e funcionam
offline.

**Sugestões de partida** aparecem no chat vazio — tela em branco não ensina o
que perguntar.
...
## Cálculo direto no chat

Quando a pergunta já traz tudo, o chat responde o número em vez de mandar
abrir a calculadora — um toque a menos, e ela costuma ter uma mão só:

| Pergunta | Resposta |
|---|---|
| dose de rocurônio pra 80 kg | 96 mg — 9,6 mL de 10 mg/mL |
| adrenalina em criança de 12 kg | 1,20 mL de 1 mg/mL |
| noradrenalina 0,1 mcg/kg/min em 70 kg | 6,6 mL/h |
| cilindro de 3,4 L a 150 kgf, fluxo 5 L/min | 1h17 |

O alerta da droga vem junto (contraindicação, diluição, teto de volume), e o
botão da ferramenta continua embaixo para ajustar.

**Quando falta dado, o chat diz qual**: "para calcular isso falta o tamanho do
cilindro, a pressão do manômetro e o fluxo" — em vez de deixar a pessoa
descobrir na tela qual campo está vazio.

Tudo isso funciona **sem a camada de IA**, e é o caminho exercitado pelos
testes: é o que ela vê quando o sinal cai.

## Conversa persistente

O histórico fica no aparelho e sobrevive à troca de aba — perder a conversa no
meio de um plantão era irritante. Botão de limpar no topo do chat.

---

## Adaptação para iPhone

O `viewport-fit=cover` faz o conteúdo ocupar a tela inteira, inclusive sob o
notch e o indicador de home. Sem tratar as áreas seguras, o topo desaparece
atrás da Dynamic Island — e isso não aparece em navegador de desktop nenhum.

O que foi tratado:

| Problema | Solução |
|---|---|
| Topo sob o notch | `padding-top` com `safe-area-inset-top` no cabeçalho e na folha |
| Notch invadindo em paisagem | `safe-area-inset-left/right` nas margens |
| Rodapé sob o indicador de home | `safe-area-inset-bottom` nas abas e no placar |
| Branco no repique da rolagem | cor de fundo no `html` |
| Texto inflando ao girar | `-webkit-text-size-adjust: 100%` |
| Puxar-para-atualizar acidental | `overscroll-behavior-y: none` |
| Espera de 300 ms no toque | `touch-action: manipulation` |
| Página rolando atrás da folha | trava com `position: fixed`, guardando e devolvendo a posição |
| Teclado cobrindo o resultado | o placar recolhe enquanto há campo em foco |

**Travamento de rolagem:** `overflow: hidden` no body não segura no Safari do
iOS — a página de trás continua rolando por baixo. Fixar a posição trava de
verdade, mas zera o scroll, então a posição é guardada e devolvida ao fechar.

**Modo instalado:** o manifest abre sem barra de navegador, em retrato, e traz
atalhos para Transporte, Registro e Dúvidas — o iOS os oferece ao segurar o
ícone. O app abre direto na aba do atalho.

### Teste

```bash
npx http-server public -p 8199 &
node testes/iphone.test.js
```
