...
function validarConteudo(itens){
  ...
  for (const it of itens){
    ...
    if (it.tipo === "pediatria"){
      if (!it.drogas?.length) e(ctx, "ficha sem drogas");
      if (!it.faixasPeso?.length) e(ctx, "ficha sem faixas de peso");
      (it.drogas || []).forEach(d => {
        ["nome", "cat", "apres", "dose", "diluente", "proporcao", "formula"].forEach(k => {
          if (!d[k]) e(ctx, `${d.nome || "?"}: falta ${k}`);
        });
        if (d.cat && !(it.categorias || []).some(c => c.id === d.cat))
          e(ctx, `${d.nome}: categoria "${d.cat}" não existe`);
        // a formula tem que avaliar para um numero positivo num caso concreto
        try {
          const v = new Function("p", "a", `"use strict"; return (${d.formula});`)(10, 3);
          if (!isFinite(v) || v < 0) e(ctx, `${d.nome}: fórmula devolve valor inválido (${v})`);
          if (d.max != null && v > d.max + 1e-9)
            e(ctx, `${d.nome}: em 10 kg a fórmula dá ${v.toFixed(2)}, acima do máximo ${d.max}`);
        } catch (err){
          e(ctx, `${d.nome}: fórmula inválida — ${err.message}`);
        }
      });
      // faixas de peso em ordem, sem buraco
      const lim = (it.faixasPeso || []).map(f => f.max);
      lim.forEach((v, k) => {
        if (k > 0 && v <= lim[k - 1]) e(ctx, "faixas de peso fora de ordem crescente");
      });
      (it.vitais || []).forEach(v => {
        const idades = v.faixas.map(f => f[0]);
        idades.forEach((x, k) => {
          if (k > 0 && x >= idades[k - 1]) e(ctx, `${v.rot}: faixas de idade devem ser decrescentes`);
        });
        if (idades.at(-1) !== 0) e(ctx, `${v.rot}: falta a faixa a partir de 0 ano`);
      });
    }
