...
async function iniciar(){
  try {
    ...
    pintaTags(); ligaAbas(); selo(); seloRegistro(); aplicaModo(); aplicaTema();
    // atalho da tela de inicio abre direto na aba certa
    const alvo = (location.hash || "").replace("#", "");
    const bAba = alvo && document.querySelector(`[data-aba=${CSS.escape(alvo)}]`);
    if (bAba) bAba.click(); else pintaLista();
    ligaVoz($("#voz-busca"), $("#q"));
  } catch (err) {
...
function resumirItem(e){
  ...
  if (e.tipo === "pediatria")
    l.push("Doses pediatricas por peso: " + e.drogas.map(d =>
      `${d.nome} ${d.dose} (${d.apres}${d.max != null ? `, max ${d.max} mL` : ""})`).join("; "));
  if (e.tipo === "oxigenio")
...
/* Escolhe os trechos mais provaveis para a pergunta.
   Antes eu usava so o resolvedor: se ele errasse, eu mandava os itens de
   Transporte por padrao, e o modelo respondia "nao sei" sobre coisa que
   estava no conteudo. Agora pontuo todos os itens pelo texto completo —
   inclusive nomes de drogas e criterios internos — e mando os melhores. */
const semAc = s => s.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();

function trechosPara(pergunta){
  const t = semAc(pergunta);
  const palavras = [...new Set(t.split(/\W+/).filter(p => p.length > 2))];

  const pontuados = TUDO.map(item => {
    const resumo = resumirItem(item);
    const alvo = semAc(resumo.nome + " " + resumo.corpo + " " + (item.sigla || ""));
    const titulo = semAc(resumo.nome);
    let p = 0;
    palavras.forEach(w => {
      if (alvo.includes(w)) p += 1;
      if (titulo.includes(w)) p += 3;   // acerto no titulo vale mais
    });
    return { item, resumo, p };
  }).sort((a, b) => b.p - a.p);

  // o item que o resolvedor apontou entra sempre, mesmo com pontuacao baixa
  const forte = resolver(pergunta, TUDO);
  const escolhidos = [];
  if (forte) escolhidos.push(resumirItem(forte.item));
  pontuados.filter(x => x.p > 0).forEach(x => {
    if (escolhidos.length < 5 && !escolhidos.some(e => e.id === x.item.id))
      escolhidos.push(x.resumo);
  });

  // nada casou: manda um panorama para o modelo poder ao menos orientar
  return escolhidos.length ? escolhidos : TUDO.slice(0, 5).map(resumirItem);
}

/* Perguntas sobre o proprio conteudo. Antes caiam em "nao encontrei",
   o que e uma resposta ruim: a informacao existe, e sobre o app. */
function respostaSobreApp(texto){
  const t = semAc(texto);
  const quer = /(o que|quais|que)\b.*(voce|vc|app|aplicativo|sabe|tem|faz|consegue|pode|conteudo|ferramenta|calcul)/.test(t)
    || /^(ajuda|help|menu|opcoes)\b/.test(t);
  if (!quer) return null;

  const porArea = {};
  TUDO.forEach(i => { (porArea[i.area] = porArea[i.area] || []).push(i); });
  const linhas = Object.entries(porArea)
    .map(([a, is]) => `${a}: ${is.map(i => i.nome).join(", ")}`).join("\n");
  return `Tenho ${TUDO.length} ferramentas no conteúdo revisado.\n\n${linhas}\n\nPergunte por qualquer uma delas, ou descreva a situação que eu abro a certa já preenchida.`;
}

/* Sugestoes de partida: tela de chat vazia nao ensina o que perguntar. */
const SUGESTOES = [
  "dose de rocurônio pra 80 kg",
  "cilindro no fluxo de 5 L/min",
  "paciente com dispneia no transporte",
  "adrenalina em criança de 12 kg",
  "o que você sabe?"
];

let conversando = false;
/* Historico curto da conversa. Sem isso, "e em 60 kg?" nao significava nada:
   cada pergunta chegava isolada ao modelo e a resolucao local.
   Persistido no aparelho: trocar de aba no meio de um plantao nao pode
   apagar a conversa. */
const CHAVE_CHAT = "escores:chat";
let historico = (() => {
  try { return JSON.parse(localStorage.getItem(CHAVE_CHAT)) || []; } catch { return []; }
})();
function salvaChat(){
  try { localStorage.setItem(CHAVE_CHAT, JSON.stringify(historico.slice(-12))); } catch {}
}

/* Junta a pergunta curta ao assunto anterior, para o resolvedor local
   conseguir apontar a ferramenta certa num acompanhamento. */
function comContexto(texto){
  const curta = texto.trim().split(/\s+/).length <= 4;
  const seguimento = /^(e |mas |entao|ent\u00e3o|agora|nesse|neste)\b/i.test(texto.trim());
  if ((curta || seguimento) && historico.length){
    const ult = historico.filter(h => h.papel === "eu").at(-1);
    // A pergunta nova vem primeiro: a extracao de numeros pega a primeira
    // ocorrencia, e em "e em 60 kg?" o peso que vale e o novo, nao o antigo.
    if (ult) return texto + " " + ult.txt;
  }
  return texto;
}
...
async function perguntarIA(texto){
  ...
  if (SAUDACOES.test(semAc(texto.trim())) || SAUDACOES.test(texto.trim())){
  ...
  // pergunta sobre o proprio conteudo: respondida na hora, sem rede
  const sobre = respostaSobreApp(texto);
  if (sobre){
    desenhaBalao("ia", sobre);
    conversando = false;
    return;
  }

  // O contexto tem que ser lido ANTES de empilhar: se empilhar primeiro,
  // comContexto encontra a propria pergunta como assunto anterior.
  const ampliada = comContexto(texto);
  historico.push({ papel: "eu", txt: texto });
  ...
  const local = resolver(ampliada, TUDO) || resolver(texto, TUDO);
  if (local) local.pergunta = ampliada;
  ...
  try {
    const r = await fetch("/api/responder", {
      ...
      body: JSON.stringify({
        pergunta: texto,
        trechos: trechosPara(ampliada),
        // exclui a ultima entrada: e a propria pergunta, enviada a parte
        historico: historico.slice(0, -1).slice(-6).map(h => ({ papel: h.papel, txt: h.txt }))
      })
    });
    ...
    historico.push({ papel: "ia", txt: d.resposta });
    salvaChat();
    if (d.fontes?.length){
...
/* Calcula direto na resposta quando a pergunta ja traz tudo que a
   ferramenta precisa. Abrir a calculadora para ver um numero que o app
   ja consegue produzir e um toque a mais — e ela costuma ter uma mao so.
   Devolve null quando falta dado: nesse caso o botao da ferramenta serve
   melhor, porque mostra qual campo esta vazio. */
function calculoDireto(r){
  const e = r.item, c = r.campos;

  if (e.tipo === "dosepeso" && c.peso && r.droga){
    const d = (e.grupos || []).flatMap(g => g.drogas).find(x => x.nome === r.droga.nome);
    if (!d) return null;
    const total = d.dose * c.peso;
    const ml = total / parseFloat(d.apres);
    return {
      txt: `${d.nome} em ${c.peso} kg: ${arred(total)} ${d.un.split("/")[0]} — ${arredML(ml)} mL de ${d.apres}.`,
      nota: d.nota
    };
  }

  if (e.tipo === "pediatria" && c.peso && r.droga){
    const d = e.drogas.find(x => x.nome === r.droga.nome);
    if (!d) return null;
    const v = avaliaDose(d.formula, c.peso, c.idade ?? 0);
    const teto = d.max != null && v >= d.max - 1e-9;
    return {
      txt: `${d.nome} em ${c.peso} kg: ${v.toFixed(2).replace(".", ",")} mL de ${d.apres} (${d.dose}).`,
      nota: (teto ? `Volume no teto de ${d.max} mL. ` : "") +
            (d.diluente === "S/ diluir" ? "Sem diluir." : `Diluir em ${d.diluente}, ${d.proporcao}.`)
    };
  }

  if (e.tipo === "infusao" && c.peso && c.dose != null && r.droga){
    const d = e.drogas.find(x => x.id === r.droga.id);
    if (!d) return null;
    const mlh = (c.dose * c.peso * (d.unidade.endsWith("/min") ? 60 : 1)) / d.conc;
    const fora = c.dose < d.min || c.dose > d.max;
    return {
      txt: `${d.nome} ${c.dose} ${d.unidade} em ${c.peso} kg: ${mlh.toFixed(1)} mL/h.`,
      nota: (fora ? `Fora da faixa usual (${d.min}–${d.max} ${d.unidade}). ` : "") + d.diluicao
    };
  }

  if (e.tipo === "oxigenio" && c.pressao && c.fluxo){
    const cap = c.capacidade ?? null;
    if (!cap) return null;
    const min = ((cap * c.pressao) / c.fluxo) * (1 - (e.reservaPadrao ?? 0) / 100);
    const h = Math.floor(min / 60), m = Math.round(min % 60);
    return {
      txt: `Cilindro de ${cap} L a ${c.pressao} kgf/cm², fluxo ${c.fluxo} L/min: ${h ? `${h}h${String(m).padStart(2, "0")}` : `${m} min`}.`,
      nota: `Já com ${e.reservaPadrao}% de reserva.`
    };
  }

  if (e.tipo === "formula"){
    const vals = { ...c };
    // "mulher de 70 anos" ja diz o sexo — pedir de novo seria burocracia
    e.campos.filter(x => x.tipo === "opcao").forEach(x => {
      if (vals[x.id] != null) return;
      const t = semAc(r.pergunta || "");
      const achou = x.opcoes.find(([rot]) => {
        const k = semAc(rot);
        return t.includes(k) ||
          (k === "feminino" && /\b(mulher|feminin|paciente do sexo f)\b/.test(t)) ||
          (k === "masculino" && /\b(homem|masculin|paciente do sexo m)\b/.test(t));
      });
      if (achou) vals[x.id] = achou[1];
    });
    if (e.campos.some(x => vals[x.id] == null)) return null;
    const v = e.calc(vals);
    const fx = faixaDe(e, v);
    return { txt: `${e.nome}: ${v.toFixed(1)} ${e.unidade}. ${fx.nome}.`, nota: fx.nota };
  }

  return null;
}

/* Diz qual dado falta, em vez de mandar a pessoa descobrir na tela. */
function faltaPara(r){
  const e = r.item, c = r.campos;
  const nomes = [];
  const pede = (cond, rot) => { if (cond) nomes.push(rot); };

  if (e.tipo === "dosepeso" || e.tipo === "pediatria"){
    pede(!c.peso, "o peso");
    pede(!r.droga, "qual droga");
  } else if (e.tipo === "infusao"){
    pede(!r.droga, "qual droga");
    pede(!c.peso, "o peso");
    pede(c.dose == null, "a dose desejada");
  } else if (e.tipo === "oxigenio"){
    pede(!c.capacidade, "o tamanho do cilindro");
    pede(!c.pressao, "a pressão do manômetro");
    pede(!c.fluxo, "o fluxo");
  } else if (e.tipo === "formula"){
    e.campos.forEach(x => pede(c[x.id] == null && x.tipo !== "opcao", x.txt.toLowerCase()));
  } else return null;
...
function semIA(texto, local){
  ...
  // Se a pergunta ja traz tudo, responde o numero em vez de mandar abrir.
  const calc = calculoDireto(r);
  if (calc){
    const b = desenhaBalao("ia", calc.txt);
    if (calc.nota){
      const n = document.createElement("div");
      n.className = "fontes"; n.textContent = calc.nota;
      b.append(n);
    }
    anexaFerramenta(b, r);
    return;
  ...
  const falta = faltaPara(r);
  const b = desenhaBalao("ia", falta
    ? `Para calcular isso falta ${falta}. A ferramenta abre com o resto preenchido:`
    : "Isso está no conteúdo:");
  anexaFerramenta(b, r);
}
...
function naoAchou(texto, local){
  ...
  if (r){
    ...
    const b = desenhaBalao("ia", falta
      ? `Para calcular isso falta ${falta}. A ferramenta abre com o resto preenchido:`
      : "Essa pergunta depende de valores que variam por paciente. A ferramenta calcula:");
    anexaFerramenta(b, r);
    return;
  }
...
/* ---------- iPhone ---------- */
/* No iOS, overflow:hidden no body nao trava a rolagem: a pagina de tras
   continua rolando por baixo da folha. Fixar a posicao trava de verdade,
   mas zera o scroll — por isso guardamos e devolvemos. */
let scrollGuardado = 0;
function travaRolagem(on){
...
/* O teclado do iOS sobe por cima do rodape fixo e cobre o resultado.
   Enquanto ha campo em foco, o placar recolhe. */
addEventListener("focusin", ev => {
  if (ev.target.matches("input, textarea")) document.body.classList.add("digitando");
});
...
/* ---------- tema ---------- */
/* Tres opcoes: automatico (segue o sistema), claro e escuro.
   O automatico e o padrao — mas plantao tem horario invertido, e o
   sistema do celular nem sempre acompanha o turno de quem trabalha
   a noite. Por isso a escolha manual existe. */
const TEMA = "escores:tema";
const TEMAS = ["auto", "claro", "escuro"];
const ROTULO = { auto: "Automático", claro: "Claro", escuro: "Escuro" };

function aplicaTema(){
  const t = localStorage.getItem(TEMA) || "auto";
  document.documentElement.dataset.tema = t;
  const b = $("#tema");
  if (b) b.setAttribute("aria-label", "Tema: " + ROTULO[t]);
...
if ($("#tema")) $("#tema").onclick = () => {
  ...
  const prox = TEMAS[(TEMAS.indexOf(atual) + 1) % TEMAS.length];
  try { localStorage.setItem(TEMA, prox); } catch {}
  aplicaTema();
  aviso("Tema: " + ROTULO[prox]);
};

/* Aviso curto no rodape — confirma a acao sem ocupar espaco permanente. */
let avisoTimer = null;
function aviso(txt){
  let el = $("#toast");
  if (!el){
...
if ($("#modo")) $("#modo").onclick = () => {
  ...
  aplicaModo();
  aviso(on ? "Modo ambulância desligado" : "Modo ambulância");
};
...
function pintaDuvidas(){
  ...
  if (historico.length){
    historico.forEach(h => desenhaBalao(h.papel === "ia" ? "ia" : "eu", h.txt));
    const lim = document.createElement("button");
    lim.className = "limpa-chat";
    lim.textContent = "Limpar conversa";
    lim.onclick = () => {
      historico = [];
      salvaChat();
      pintaDuvidas();
    };
    chat.querySelector(".thread").append(lim);
  } else {
  ...
  // sugestoes: tela vazia nao ensina o que perguntar
  if (!historico.length){
    ...
    SUGESTOES.forEach(txt => {
      const b = document.createElement("button");
      b.textContent = txt;
      b.onclick = () => { sug.remove(); perguntarIA(txt); };
      sug.append(b);
    });
    chat.querySelector(".thread").append(sug);
  }
...
function abrir(e, intencao){
  ...
  else if (e.tipo === "pediatria"){
    montaPediatria(e, box);
  }

  if (intencao) preencher(e, intencao);

  $("#sheet").classList.add("on");
  document.body.classList.add("em-ferramenta");
  travaRolagem(true);
  calcular();
}
...
function calcular(){
  ...
  if (e.tipo === "pediatria") return;   // a ficha se atualiza sozinha
...
$("#back").onclick = () => {
  ...
  document.body.classList.remove("em-ferramenta");
  travaRolagem(false);
  atual = null; fluxo = null;
};
$("#q").addEventListener("input", () => {
  document.querySelector(".search").classList.toggle("tem-texto", !!$("#q").value);
  pintaLista(); pintaResposta();
});
$("#limpar").onclick = () => {
  $("#q").value = "";
