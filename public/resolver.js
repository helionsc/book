...
const SINONIMOS = {
  ...
  "pcr-pediatrica": ["pediatrica","pediatria","crianca","criança","bebe","lactente",
    "parada pediatrica","pcr infantil","broselow","dose por peso","recem nascido","neonato"],
  "dif-dispneia": ["dispneia","falta de ar","desconforto respiratorio","cansaco",
    "nao consegue respirar","taquipneia","diferencial de dispneia","hipoteses dispneia"],
  "dif-hipotensao": ["hipotensao","pressao caiu","pa baixa","choque","instavel",
    "hipotenso","pam baixa","diferencial de hipotensao"],
...
function extrairCampos(txt){
  ...
  return {
    ...
    pressao:  num(/(\d+)\s*(?:kgf|bar|psi)/) ?? num(/(?:manometro|pressao)\s*(?:de\s*|:)?\s*(\d+)/),
    ...
    // "cilindro de 3,4 L", "torpedo 10 L" — capacidade hidraulica do cilindro
    capacidade: num(/(?:cilindro|torpedo)\s*(?:de\s*)?(\d+[.,]?\d*)\s*l\b/)
  };
}
...
function acharDroga(item, txt){
  const t = semAcento(txt);
  const lista = item.tipo === "infusao" || item.tipo === "pediatria"
    ? (item.drogas || []).map(d => ({ id: d.id || d.nome, nome: d.nome }))
    : (item.grupos || []).flatMap(g => g.drogas.map(d => ({ id: d.nome, nome: d.nome })));
  return lista.find(d => t.includes(semAcento(d.nome).split(" ")[0])) || null;
}

/* Palavras que aparecem em quase todo item do conteudo. Pontuar por elas
   faz um item generico vencer outro que casou pelo termo que importa
   ("paciente com dispneia no transporte" casava por "paciente"). */
/* Marcadores de que a pergunta e sobre crianca. */
const PEDIATRICO = ["pediatrica","pediatrico","pediatria","crianca","criança","bebe","bebê",
  "lactente","infantil","neonato","recem nascido","rn","escolar","pre-escolar"];

const VAGAS = new Set(["paciente","transporte","remocao","ambulancia","durante",
  "adulto","aguda","agudo","medico","medica","qual","como","quanto","fazer","preciso"]);
...
function resolver(pergunta, itens){
  ...
  const palavras = t.split(/\W+/).filter(p => p.length > 2 && !VAGAS.has(p));

  const pontuados = itens.map(item => {
    let p = 0;
    const alvo = semAcento(`${item.nome} ${item.sigla || ""} ${item.uso}`);

    /* Um termo pontua uma vez so. Sem isso, "hipotensao" valia como sinonimo
       E como palavra do nome do item, dando 14 contra 10 de um item que
       casou igualmente bem — o empate era decidido por duplicidade. */
    const jaContou = new Set();
    (SINONIMOS[item.id] || []).forEach(s => {
      if (contemTermo(t, s)){ p += 10; jaContou.add(semAcento(s)); }
    });
    palavras.forEach(w => { if (alvo.includes(w) && !jaContou.has(w)) p += 4; });
    ...
    /* Contexto pediatrico e decisivo em dose. "dose de adrenalina pediatrica"
       casava com a bomba de adulto pelo nome da droga (+12) e vencia a ficha
       pediatrica (+10) — a troca exata que causa dose errada em crianca.
       Quando a pergunta traz marcador pediatrico, a ficha pediatrica ganha. */
    if (item.tipo === "pediatria" && PEDIATRICO.some(w => contemTermo(t, w))) p += 26;

    return { item, p };
  }).filter(x => x.p > 0).sort((a, b) => {
    if (b.p !== a.p) return b.p - a.p;
    /* Empate: diferencial e a resposta para "nao sei o que e isso". Quando a
       pergunta ja nomeia a suspeita ("paciente septico com hipotensao"), a
       ferramenta especifica serve melhor. Desempate na ordem, nunca na
       pontuacao — subtrair pontos derrubava acertos validos abaixo do piso. */
    const dif = x => x.item.tipo === "diferencial" ? 1 : 0;
    return dif(a) - dif(b);
  });
