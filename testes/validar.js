...
for (const c of casos) {
  ...
  if (c.diferencial) {
    const e = acha(c.diferencial);
    if (!e) { erro(`${c.descricao}: diferencial "${c.diferencial}" nao existe`); continue; }
    const h = e.hipoteses.find(x => x.nome === c.hipotese);
    if (!h) { erro(`${c.descricao}: hipotese "${c.hipotese}" nao encontrada`); continue; }
    if (h.risco !== c.riscoEsperado) {
      erro(`${c.descricao}\n           risco esperado "${c.riscoEsperado}", obtido "${h.risco}"`);
      continue;
    }
    if (c.posicao !== undefined && e.hipoteses.indexOf(h) !== c.posicao) {
      erro(`${c.descricao}\n           esperada na posicao ${c.posicao}, esta na ${e.hipoteses.indexOf(h)}`);
      continue;
    }
    passa(c.descricao);
  }
  ...
  if (c.pediatria) {
    const e = acha(c.pediatria);
    if (!e) { erro(`${c.descricao}: ficha "${c.pediatria}" nao existe`); continue; }
    const d = e.drogas.find(x => x.nome === c.droga);
    if (!d) { erro(`${c.descricao}: droga "${c.droga}" nao encontrada`); continue; }
    let v;
    try { v = new Function("p", "a", `"use strict"; return (${d.formula});`)(c.peso, c.anos); }
    catch (err) { erro(`${c.descricao}: formula invalida — ${err.message}`); continue; }
    if (!perto(v, c.esperadoMl, c.tolerancia)) {
      erro(`${c.descricao}\n           esperado ${c.esperadoMl} mL, obtido ${Number(v.toFixed(3))}`);
      continue;
    }
    passa(c.descricao);
  }
...
const testados = new Set(casos.map(c => c.escore || c.infusao || c.oxigenio || c.dosepeso || c.fluxograma || c.diferencial || c.pediatria));
